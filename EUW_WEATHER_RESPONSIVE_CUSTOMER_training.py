...
# getting usage of all the customers
    ...
    df1 = Usage.alias("df1")
    df2 = customer_df.alias("df2")
    Usagev1 = df1.join(df2,"CUSTOMER_ID").select("df1.*").distinct()
    Usagev1_Count=Usagev1.count()
    fileLog(Usagev1_Count)

    Usagev1.createOrReplaceTempView("Usagev1_temp")
    spark.sql("drop table if exists "+db_analytical_temp+".EUW_humidity_responsive_customers_Usage_temp")
    spark.sql("create table "+db_analytical_temp+".EUW_humidity_responsive_customers_Usage_temp as select * from Usagev1_temp")
    column_values=[]
